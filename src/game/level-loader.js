...
import { CORNER, DIRECTION, GAME_STATE } from "./constants.js";

function resolveLevelFileName(game, level) {
  const key = String(Math.max(0, Math.floor(Number(level) || 0)));
  const mapped = game.levelFileIndex?.[key];
  if (typeof mapped === "string" && mapped.endsWith(".md")) return mapped;
  return `${String(Number(key) || 0).padStart(4, "0")}.md`;
}
...
export function startLevelFlow(game, nowTs) {
  ...
  game.gameState = GAME_STATE.LEVEL_BANNER;
...
export async function ensureLevelSongLoaded(game, level) {
  ...
  const p = (async () => {
    const fileName = resolveLevelFileName(game, level);
    const url = `/src/levels/${fileName}`;
    ...
    for (const e of events) {
      e.instrument = String(e.instrument || game.defaultInstrument)
        .trim()
        .toLowerCase();
    }
...
export function getNoteSpawnPoint(layout, corner) {
  if (corner === CORNER.TOP_LEFT) {
    return {
      x: layout.centers[DIRECTION.LEFT].x,
      y: layout.centers[DIRECTION.UP].y
    };
  }
  if (corner === CORNER.TOP_RIGHT) {
    return {
      x: layout.centers[DIRECTION.RIGHT].x,
      y: layout.centers[DIRECTION.UP].y
    };
  }
  if (corner === CORNER.BOTTOM_LEFT) {
    return {
      x: layout.centers[DIRECTION.LEFT].x,
      y: layout.centers[DIRECTION.DOWN].y
    };
  }
  return {
    x: layout.centers[DIRECTION.RIGHT].x,
  ...
  while (game.songEventIndex < game.songEvents.length) {
    ...
    if (e.soundOnly || e.lane === 0) {
      game.playNote(
        ...
        e.durationMs ?? 220,
        e.velocity ?? 0.85,
        e.instrument ?? game.defaultInstrument,
        e.effects ?? null
      );
      game.songEventIndex++;
      continue;
    ...
    game.notes.push({
      ...
      instrument: e.instrument ?? game.defaultInstrument,
