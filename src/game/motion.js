...
export function updateSquareMotionBaselines(game, data, width, height, layout) {
  if (
    game.gameState === GAME_STATE.TITLE ||
...
export function updateMouseActivation(game, layout, nowTs) {
  ...
  let dir = null;
  if (row === 0 && col === 1) dir = DIRECTION.UP;
  else if (row === 1 && col === 0) dir = DIRECTION.LEFT;
  else if (row === 1 && col === 1) dir = DIRECTION.MIDDLE;
  else if (row === 1 && col === 2) dir = DIRECTION.RIGHT;
  else if (row === 2 && col === 1) dir = DIRECTION.DOWN;
  else dir = null;
