import { GAME_MODE, PLAY_MODE } from "./constants.js";
...
export function resolveNoteHits(game, nowTs) {
  ...
  const noteIsHitInGraceWindow = (note) => {
    ...
    if (
      game.play_mode === PLAY_MODE.TOUCH &&
  ...
  if (game.GAME_MODE === GAME_MODE.DEV_RECORD) {
    for (const note of resolving) {
      ...
      game.playNote(
        ...
        note.durationMs ?? 220,
        note.velocity ?? 0.85,
        note.instrument ?? game.defaultInstrument,
        note.effects ?? null
      );
    }
    return;
  ...
  if (allHit) {
    ...
    const powerModeThreshold = Number.isFinite(game.scoreForceModeThreshold)
      ...
      : 100;
    if (
      game.GAME_MODE !== GAME_MODE.DEV_RECORD &&
      !game.powerMode &&
...
export function spawnScorePopups(game, group, perNotePoints, nowTs) {
  ...
  for (const {
    ...
    const dirColor = game.powerMode
      ...
      : game.colors[dir] || game.visualDefaults?.popupColor;
...
export function spawnCaptureEffect(game, note, nowTs) {
  ...
  const color = game.powerMode
    ? game.powerColors?.[note.targetDir] || game.colors?.[note.targetDir]
    : game.colors[note.targetDir] || game.visualDefaults?.captureColor;
  const wavePoints = 42;
